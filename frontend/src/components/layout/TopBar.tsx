...
import { Menu, Server, ShieldCheck, Clock } from 'lucide-react';
...
export const TopBar: React.FC<TopBarProps> = ({ onToggleSidebar }) => {
  ...
  return (
    <header className="h-16 bg-[#111722] border-b border-[#232E42] px-4 lg:px-6 flex items-center justify-between sticky top-0 z-30">
      {/* Left side: Hamburger + Page Title */}
      <div className="flex items-center space-x-3">
        <button
          onClick={onToggleSidebar}
          className="lg:hidden p-2 text-slate-400 hover:text-white rounded hover:bg-[#161F2E]"
        ...
        <div className="hidden sm:block">
          ...
          <h2 className="text-xs md:text-sm font-semibold text-slate-100 font-mono truncate max-w-xs md:max-w-md">
            {currentTitle}
          </h2>
        </div>
      </div>

      {/* Right side: Asset selector + Telemetry + MAID Profile */}
      <div className="flex items-center space-x-2 md:space-x-3">
        {/* Asset Quick Switcher */}
        <div className="flex items-center bg-[#161F2E] border border-[#232E42] rounded p-0.5 text-xs font-mono">
          <button
          ...
          <button
            ...
            className={`px-2 py-1 rounded transition-colors ${
              selectedAsset === 'bitcoin'
                ? 'bg-orange-950/80 text-orange-400 border border-orange-700/60 font-semibold'
                : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            BTC
          ...
          <button
            ...
            className={`px-2 py-1 rounded transition-colors ${
              selectedAsset === 'nvidia'
                ? 'bg-lime-950/80 text-lime-400 border border-lime-700/60 font-semibold'
                : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            NVDA
        ...
        {/* UTC Clock */}
        <div className="hidden xl:flex items-center space-x-1.5 bg-[#161F2E] border border-[#232E42] px-2.5 py-1 rounded text-xs font-mono text-slate-400">
        ...
        <div
          className="flex items-center space-x-1.5 bg-[#161F2E] border border-[#232E42] px-2.5 py-1 rounded text-xs font-mono"
        ...
        <NavLink
          to="/login"
          className="flex items-center space-x-2 bg-[#161F2E] hover:bg-[#1E2A3E] border border-[#232E42] px-2.5 py-1 rounded text-xs font-mono text-slate-300 transition-colors"
        >
          ...
          <span className="hidden md:inline text-slate-300">MAID Auth</span>
